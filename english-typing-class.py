...
def get_translation(phrase):
    try:
        print(phrase['French'])
        user_answer = input("Translate this to English: ")
        # Remove punctuation from user_answer and phrase['English']
        user_answer = user_answer.lower().translate(str.maketrans('', '', string.punctuation))
        correct_answer = phrase['English'].lower().translate(str.maketrans('', '', string.punctuation))
        return user_answer, correct_answer
    except KeyError:
        print("An error occurred. Please make sure the phrase dictionary has 'French' and 'English' keys.")
        return None, None
...
def check_translation(user_answer, correct_answer, phrase):
    if user_answer == correct_answer:
        ...
        if similarity > 95:
            print(f"Good Job, Correct answer: {phrase['English']}")
        # If the similarity is above 80, consider the answer "close"
        elif similarity > 80:
            print(f"Almost there. Correct Answer: {phrase['English']}")
        else:
            print(f"Not quite, Correct Answer: {phrase['English']}")
